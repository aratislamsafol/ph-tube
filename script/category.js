...
function categoryLoad(data){
    ...
    data.forEach(element => {
        console.log(element)
        category.innerHTML += 
        `<button class="p-2 bg-blue-500 hover:bg-blue-700 text-white rounded text-sm" onclick="loadCategoryVideo(${element.category_id})" type="button" >${element.category}</button>
        `
    });
}
